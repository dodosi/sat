...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_array()) {
        ...
        $updateLink = '<a href="updateQuestionAndChoice.php?row_counter=' . $row[0] . '&name=' . $row[1] . '&title=' . $row[2] . '&type=' . $row[3] . '&question_name=' . $row[4] . '&choice_name=' . $row[5] . '&is_required=' . $row[6] .'&flag=' . $row[7] .'&description=' . $row[8] .'&groupe=' . $row[9] .'&category=' . $row[10] .'&visible_if=' . $row[11].'&question=' . $row[12].'">Update</a>';
